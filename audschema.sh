...
# Get all table names for the target schema
echo "Fetching tables for schema: ${TARGET_SCHEMA}..."
TABLE_NAMES=$(db2 -x "SELECT TABNAME FROM SYSCAT.TABLES WHERE TABSCHEMA = '${TARGET_SCHEMA}' AND TYPE = 'T' ORDER BY TABNAME" \
    | sed 's/^[ \t]*//;s/[ \t]*$//' | sed '/^$/d')
...
for TABLE_NAME in $TABLE_NAMES; do
    ...
    | awk '
    {
        ...
        if (length(type) > 9) type = substr(type, 1, 9);
        if (length(colname) > 16) colname = substr(colname, 1, 16);
        if (length(remarks) > 30) remarks = substr(remarks, 1, 30); # Adjust max length for remarks

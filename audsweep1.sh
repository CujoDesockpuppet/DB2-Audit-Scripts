...
# Define the recipient(s) for email notifications.
# Separate multiple addresses with a comma.
MAIL_TO="kevin_fries@colpal.com" # <--- IMPORTANT: Change this to your actual test email address!
# If sendmail is not running, the mail will go to the $HOME/dead.letter file
# This also applies of the user above in the "MAIL_TO" 
# Example for multiple recipients:
# MAIL_TO="Kevin_fries@colpal.com,esc_dba_group@colpal.com,esc_prod_control@colpal.com"
...
# Create a temp log file in /tmp. This must be defined early.
...
# This function is moved to the top so it's defined before any calls to it.
exit_with_log_move() {
    local exit_code="$1"
    local log_dir_for_final="$2" # Renamed to avoid confusion with local LOGDIR variable
    local temp_log_file_path="$3"
    local script_name="$4"
    ...
    local FINAL_LOGFILE="$log_dir_for_final/${script_name}_${TIMESTAMP}.log"

    # Ensure the final log directory exists before moving
    if [ ! -d "$log_dir_for_final" ]; then
        mkdir -p "$log_dir_for_final" 2>/dev/null # Suppress error if already exists
    fi

    # Move the temporary log file to its final destination
    if mv "$temp_log_file_path" "$FINAL_LOGFILE" ; then
        echo "Log written to $FINAL_LOGFILE"
    else
        echo "Error: Failed to move log from '$temp_log_file_path' to '$FINAL_LOGFILE'. Log content below:"
        cat "$temp_log_file_path" # Print temp log content to console if move fails
    fi

    # --- Emailing Logic ---
    if [ "$exit_code" -ne 0 ]; then
        echo "Non-zero exit code ($exit_code) detected. Sending email notification."
        # Ensure CUR_SID and CUR_HOST are defined for the email subject,
        # especially for early exits where they might not be initialized.
        local current_sid="${CUR_SID:-UNKNOWN_SID}"
        local current_host="${CUR_HOST:-$(hostname)}"
        local EMAIL_SUBJECT="ERROR: ${current_sid} on ${current_host} Audit Archive Report - Exit Code $exit_code"
        local EMAIL_BODY_FILE="$FINAL_LOGFILE" # Use the final moved log file for the email body

        # Check if the final log file exists before emailing
        if [ -f "$EMAIL_BODY_FILE" ]; then
            # Using mailx for better control over subject and body
            # Piping the content of the log file directly to mailx
            cat "$EMAIL_BODY_FILE" | mailx -s "$EMAIL_SUBJECT" "$MAIL_TO"
            if [ $? -eq 0 ]; then
                echo "Email sent successfully to $MAIL_TO with subject: '$EMAIL_SUBJECT'"
            else
                echo "Error: Failed to send email notification."
            fi
        else
            echo "Error: Log file '$EMAIL_BODY_FILE' not found for email body. Email not sent."
            # Fallback to temporary log file if the final one isn't found (e.g., if mv failed)
            if [ -f "$temp_log_file_path" ]; then
                echo "Attempting to email content of temporary log file as fallback."
                cat "$temp_log_file_path" | mailx -s "$EMAIL_SUBJECT (Fallback Log)" "$MAIL_TO"
                if [ $? -eq 0 ]; then
                    echo "Fallback email sent successfully from temporary log file."
                else
                    echo "Error: Fallback email failed."
                fi
            fi
        fi
    else
        echo "Script completed successfully (exit code 0). No email notification needed."
    fi
    # --- End Emailing Logic ---
...
if [ "$#" -lt 1 ]; then
    ...
    # --- START: Mailx Test Block for Missing Argument ---
    echo "--------------------------------------------------------------------------------------------------"
    echo "--- Initiating Mailx Test: Script called without SID argument. ---"
    echo "--- An email should be sent with this log content. ---"
    echo "--------------------------------------------------------------------------------------------------"
    # Temporarily set CUR_SID and CUR_HOST for the test email subject if not already set
    # This ensures the email subject has some context even if the script exits very early.
    CUR_SID="${CUR_SID:-TEST_SID}" # Use existing CUR_SID or default to TEST_SID
    CUR_HOST="${CUR_HOST:-$(hostname)}" # Use existing CUR_HOST or default to current hostname
    # For this test, we'll force an exit with a specific test code (e.g., 99)
    # Use /tmp as the log_dir for this very early exit, as LOGDIR might not be established yet.
    exit_with_log_move 99 "/tmp" "$TEMP_LOGFILE" "$0" "$SCRIPT_PATH" # Exit with a test code
    # --- END: Mailx Test Block for Missing Argument ---

    exit 1 # Original exit, though the test block above will cause an exit first
fi
...
# The SUBJECT variable below is now primarily for successful runs or general logging.
# The actual email subject for errors will be dynamically set in exit_with_log_move.
SUBJECT="${CUR_SID} on ${CUR_HOST} Audit Archive Report"

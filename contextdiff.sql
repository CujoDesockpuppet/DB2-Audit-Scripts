 WITH CONTEXT_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'AUDIT' AND TABNAME = 'CONTEXT'
    ORDER BY COLNO
),
CONTEXT_NEW_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'DB2CAI' AND TABNAME = 'CONTEXT_NEW'
    ORDER BY COLNO
)
SELECT
    ...
    CASE
        WHEN s1.COLNO IS NULL THEN 'CONTEXT_NEW_TABLE'
        WHEN s2.COLNO IS NULL THEN 'CONTEXT_TABLE'
        ELSE 'Both'
    END AS Difference_Source
FROM CONTEXT_TABLE s1
FULL OUTER JOIN CONTEXT_NEW_TABLE s2

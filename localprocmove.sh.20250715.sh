...
validate_date() {
    ...
    if [[ "$date_str" =~ ^[0-9]{8}$ ]]; then
        # Further validate if it's a real date (e.g., using date -d)
        # This requires GNU date. For AIX, a more complex check would be needed.
        # For simplicity, we'll rely on the regex for now.
        return 0 # Valid
    else
        return 1 # Invalid
    fi
...
echo "Extracting files from $START_DATE to $END_DATE (inclusive of timestamps within these dates)."
echo "--------------------------------------------------------"
...
done < <(find "$PROCESSED_ARCHIVE_DIR" -type f -name "db2audit.*.log.*" 2>/dev/null | sort)

if [ ${#ALL_ARCHIVED_FILES[@]} -eq 0 ]; then
    echo "No raw archived audit files found in '$PROCESSED_ARCHIVE_DIR'. Nothing to extract or load in this run."
...
EARLIEST_FILE=""
LATEST_FILE=""
FOUND_START_RANGE_FILE=0
FOUND_END_RANGE_FILE=0

# Determine the files to process based on date range
for file in "${ALL_ARCHIVED_FILES[@]}"; do
    # Extract the timestamp part from the filename (e.g., 20250708031337)
    filename=$(basename "$file")
    # Use awk to reliably get the last numeric part after the last dot
    file_timestamp=$(echo "$filename" | awk -F'.' '{print $NF}')
    file_date=${file_timestamp:0:8} # YYYYMMDD part of the timestamp

    # Set earliest and latest file (needed for edge cases)
    if [ -z "$EARLIEST_FILE" ]; then
        EARLIEST_FILE="$file"
    fi
    LATEST_FILE="$file"

    # Check if file is within the desired date range
    if [[ "$file_date" -ge "$START_DATE" ]] && [[ "$file_date" -le "$END_DATE" ]]; then
        if [ "$FOUND_START_RANGE_FILE" -eq 0 ]; then
            # This is the first file found within the range, but we need the one just BEFORE it
            # Add the previous file if it exists and hasn't been added yet
            local previous_file_index=$(expr ${#ARCHIVED_FILES_TO_PROCESS[@]} - 1)
            if [ "$previous_file_index" -ge 0 ] && [ "${ARCHIVED_FILES_TO_PROCESS[$previous_file_index]}" != "$file" ]; then
                ARCHIVED_FILES_TO_PROCESS+=("${ALL_ARCHIVED_FILES[$previous_file_index]}")
            fi
            FOUND_START_RANGE_FILE=1
        fi
        ARCHIVED_FILES_TO_PROCESS+=("$file")
    else
        # If we've found files within the range and now we are past the end date,
        # add the current file (which is just after the end range)
        if [ "$FOUND_START_RANGE_FILE" -eq 1 ] && [ "$FOUND_END_RANGE_FILE" -eq 0 ]; then
            ARCHIVED_FILES_TO_PROCESS+=("$file")
            FOUND_END_RANGE_FILE=1
        fi
    fi
done

# Handle edge cases:
# If no files were found within the range, but files exist, grab earliest and latest.
if [ ${#ARCHIVED_FILES_TO_PROCESS[@]} -eq 0 ]; then
    if [ -n "$EARLIEST_FILE" ] && [ -n "$LATEST_FILE" ]; then
        echo "No files found within the specified date range. Extracting earliest and latest available files instead."
        ARCHIVED_FILES_TO_PROCESS=("$EARLIEST_FILE" "$LATEST_FILE")
    else
        echo "No archived audit files found in '$ARCHIVE_DIR' to process. Exiting."
        exit_script 0
    fi
fi

# Ensure unique files and re-sort in case of manual additions for edge cases
...
if [ $extract_status -eq 0 ]; then
    ...
    # I'm commenting out this section. If you wish to re-enable, uncomment the lines below.

WITH OBJMAINT_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'AUDIT' AND TABNAME = 'OBJMAINT'
    ORDER BY COLNO
),
OBJMAINT_NEW_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'DB2CAI' AND TABNAME = 'OBJMAINT_NEW'
    ORDER BY COLNO
)
SELECT
    ...
    CASE
        WHEN s1.COLNO IS NULL THEN 'OBJMAINT_NEW_TABLE'
        WHEN s2.COLNO IS NULL THEN 'OBJMAINT_TABLE'
        ELSE 'Both'
    END AS Difference_Source
FROM OBJMAINT_TABLE s1
FULL OUTER JOIN OBJMAINT_NEW_TABLE s2

WITH SECMAINT_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'AUDIT' AND TABNAME = 'SECMAINT'
    ORDER BY COLNO
),
SECMAINT_NEW_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'DB2CAI' AND TABNAME = 'SECMAINT_NEW'
    ORDER BY COLNO
)
SELECT
    ...
    CASE
        WHEN s1.COLNO IS NULL THEN 'SECMAINT_NEW_TABLE'
        WHEN s2.COLNO IS NULL THEN 'SECMAINT_TABLE'
        ELSE 'Both'
    END AS Difference_Source
FROM SECMAINT_TABLE s1
FULL OUTER JOIN SECMAINT_NEW_TABLE s2

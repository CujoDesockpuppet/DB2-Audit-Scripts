WITH SYSADMIN_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'AUDIT' AND TABNAME = 'SYSADMIN'
    ORDER BY COLNO
),
SYSADMIN_NEW_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'DB2CAI' AND TABNAME = 'SYSADMIN_NEW'
    ORDER BY COLNO
)
SELECT
    ...
    CASE
        WHEN s1.COLNO IS NULL THEN 'SYSADMIN_NEW_TABLE'
        WHEN s2.COLNO IS NULL THEN 'SYSADMIN_TABLE'
        ELSE 'Both'
    END AS Difference_Source
FROM SYSADMIN_TABLE s1
FULL OUTER JOIN SYSADMIN_NEW_TABLE s2

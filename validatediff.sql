WITH VALIDATE_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'AUDIT' AND TABNAME = 'VALIDATE'
    ORDER BY COLNO
),
VALIDATE_NEW_TABLE AS (
    SELECT COLNO, SUBSTR(COLNAME,1,20) AS ColumnName, SUBSTR(TYPENAME,1,20) AS Type,
           LENGTH, SCALE 
    FROM syscat.columns 
    WHERE TABSCHEMA = 'DB2CAI' AND TABNAME = 'VALIDATE_NEW'
    ORDER BY COLNO
)
SELECT
    ...
    CASE
        WHEN s1.COLNO IS NULL THEN 'VALIDATE_NEW_TABLE'
        WHEN s2.COLNO IS NULL THEN 'VALIDATE_TABLE'
        ELSE 'Both'
    END AS Difference_Source
FROM VALIDATE_TABLE s1
FULL OUTER JOIN VALIDATE_NEW_TABLE s2
